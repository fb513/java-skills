...
1. 运行 `git status --short` 识别工作区变更，区分本次任务相关改动和无关改动
2. 运行 `git diff HEAD` 获取本次所有新增/修改的代码（包含已暂存和未暂存的变更）
3. 调用 `java-coding-standards` 技能加载编码规范
4. 逐文件审查本次任务相关的 Java、XML、DTO diff 内容，检查是否符合规范；无关改动只记录，不修改
5. 重点关注：
   ...
   - 响应格式是否使用 CommResp
6. 发现本次任务相关问题直接修复，不只是报告；无关改动只报告风险，不碰文件
